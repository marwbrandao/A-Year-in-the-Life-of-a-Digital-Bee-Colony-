...
def plot_confidence_bar(names, means, std_devs, N, title, x_label, y_label, confidence, filename, show=False, colors=None, yscale=None):
...
def plot_bar(names, results, y_label, x_label, title="Survival Rate", colors=None, yscale=None, filename="survival_rate", show=True):
    
    fig, ax = plt.subplots()
    x_pos = np.arange(len(names))
    ax.bar(x_pos, results, align='center', alpha=0.5, color=colors if colors is not None else "gray", ecolor='black', capsize=10)
    ax.set_ylabel(y_label)
    ax.set_xlabel(x_label)
    ax.set_xticks(x_pos)
    ax.set_xticklabels(names)
...
def compare_results(results, confidence=0.95, title="Agents Comparison", metric="Steps Per Episode", colors=None):
    ...
    plot_confidence_bar(
        ...
        N=N,
        title=title,
        x_label="", y_label=f"Avg. days survived per episode",
        confidence=confidence, show=True, colors=colors, filename="results.png"
    )
    plt.savefig

def plot_confidence_errors(x, y, yerr, title, xlabel, ylabel, color, show):
